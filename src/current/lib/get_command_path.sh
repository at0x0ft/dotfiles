...
get_command_path() {
  ...
  local readonly is_valid_command="${SCRIPT_ROOT}/is_valid_command.sh"
  if ! $(${is_valid_command} "${exec_command}"); then
    return 1
  fi

  local readonly get_login_shell="${CURRENT_LIBRARY_SCRIPTS}/get_login_shell.sh"
  local readonly login_shell=$(get_login_shell)

  ${login_shell} -l -c "which ${exec_command}"
  return 0
}
get_command_path "${@}"
